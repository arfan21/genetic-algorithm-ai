...
def main():
    ...
    print(f"best index : {parent1}")
    print(f"best kromosom : {populasi[parent1]}")
    parent2 = parentSelection(allFitness)
    for fit in allFitness:
        [[key, item]] = fit.items()
        if key == parent2:
            print(f"best fitness : {fit}")
    print(f"best index : {parent2}")
    print(f"best kromosom : {populasi[parent2]}")
    print("\ncrossover :")
    print(
        f"kromosom sebelum di crossover: \nkromosom 1 : {populasi[parent1]}\nkromosom 2 : {populasi[parent2]}")
    offspring1, offspring2 = crossover(
        populasi[parent1], populasi[parent2])
    print(
        f"kromosom sesudah di crossover: \nkromosom 1 : {offspring1}\nkromosom 2 : {offspring2}")
    print("\nmutasi : ")
    print(f"kromosom sebelum di mutasi : {offspring1}")
    offspring1 = mutasi(offspring1)
    print(f"kromosom sesudah di mutasi : {offspring1}")
    print(f"kromosom sebelum di mutasi : {offspring2}")
    offspring2 = mutasi(offspring2)
    print(f"kromosom sesudah di mutasi : {offspring2}")
    print("\npopulasi baru untuk next gen dari fungsi seleksiSurvivor: ")
    newPopForNextGen = seleksiSurvivorForNextGen(
        populasi, newPopulasi, allFitness)
    for newPop in newPopForNextGen:
        print(newPop)
